...
import { TrendingUp, Plus, Trash2, Star, BarChart3, AlertCircle } from 'lucide-react';
...
export default function Stock() {
  ...
  const validateStock = (symbol) => {
    ...
    return /^[A-Z]{1,5}$/.test(trimmed) ? trimmed : null;
  };
  ...
  const handleAddStock = async () => {
    ...
    if (!validated) return setError('Stock symbol must be 1–5 uppercase letters only.');
    if (watchlist.find(stock => stock.name === validated)) return setError(`${validated} is already in your watchlist.`);
    setLoading(true);
    try {
      const res = await fetch(`${API_BASE}/stocks`, {
        method: 'POST',
      ...
      if (res.status === 201 || res.status === 200) {
        ...
        setWatchlist(prev => {
          const exists = prev.find(s => s.name === validated);
          if (exists) return prev.map(s => s.name === validated ? body.stock : s);
          return [...prev, body.stock];
        });
        setStockInput('');
      } else {
  ...
  const handleRemoveStock = async (symbol) => {
    try {
      ...
      setWatchlist(prev => prev.filter(s => s.name !== symbol));
  ...
  const handleClearAll = async () => {
    ...
    try {
      await Promise.all(watchlist.map(stock => fetch(`${API_BASE}/stocks/${stock.name}`, { method: 'DELETE' })));
      setWatchlist([]);
  ...
  const filteredWatchlist = watchlist.filter(stock => stock.name.toLowerCase().includes(searchTerm.toLowerCase()));
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
          ...
          {fetching ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {filteredWatchlist.map(stock => (
                <div key={stock.name} className="bg-slate-800/50 rounded-xl border border-slate-700/50 p-5 hover:border-blue-500/50 transition-all group">
                  <div className="flex items-start justify-between mb-3">
                    <h3 className="text-lg font-bold text-white">{stock.name}</h3>
                    <button onClick={() => handleRemoveStock(stock.name)} className="p-2 hover:bg-red-500/10 rounded-lg transition-all opacity-0 group-hover:opacity-100" title="Remove from watchlist">
                      <Trash2 className="w-4 h-4 text-red-400" />
                    </button>
                  </div>
                  <p className="text-slate-400 text-sm">
                    Price: ₹{stock.price !== undefined ? stock.price.toFixed(2) : 'N/A'}
                  </p>
                </div>
              ))}
            </div>
        ...
        <div className="mt-8 text-center">
          <p className="text-xs text-slate-500">
            💡 Connected to backend API — live MongoDB integration
          </p>
        </div>
      </div>
    </div>
